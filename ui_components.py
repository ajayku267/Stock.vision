"""
Modular UI components for StockVision Streamlit application.
"""
...
def render_kpi_cards(config: Dict[str, Any], metrics: Dict[str, float], signal_info: Dict[str, Any]):
    """Render KPI metric cards."""
    top_c1, top_c2, top_c3, top_c4 = st.columns(4)
    top_c1.metric("Ticker", config["selected_stock"])
    top_c2.metric("Training Rows", f"{metrics.get('training_rows', 0):,}")
    top_c3.metric("Forecast Horizon", f"{config['years_to_predict']} yr")
    top_c4.metric("MAPE", f"{metrics.get('mape', 0):.2f}%")


def render_dataframes_tab(selected_stock: str, start_date: date, end_date: date, 
                         new_data: pd.DataFrame, new_forecast: pd.DataFrame, period: int):
    """Render Dataframes tab content."""
    st.markdown(f"<h2><span style='color: orange;'>{selected_stock}</span> Historical Data</h2>", unsafe_allow_html=True)
    st.write(f"This section displays historical stock price data for {selected_stock} from {start_date} to {end_date}.")
    ...
    st.write(f"This section displays the forecasted stock price data for {selected_stock} using the Prophet model from {end_date} to {end_date + timedelta(days=period)}.")
...
def render_plots_tab(data: pd.DataFrame):
    """Render Plots tab content."""
    from services import plot_data, plot_volume
    plot_data(data)
    plot_volume(data)


def render_statistics_tab(selected_stock: str, stats_summary_df: pd.DataFrame, tech_data: pd.DataFrame):
...
def render_forecasting_tab(selected_stock: str, end_date: date, period: int, metrics: Dict[str, float],
                          signal_info: Dict[str, Any], next_day_prediction: float, backtest: Dict[str, Any],
                          forecast: pd.DataFrame, model, df_train: pd.DataFrame):
    ...
    c1.metric("Signal", signal_info.get("signal", "UNKNOWN"))
    c2.metric("Expected Return", f"{signal_info.get('expected_return_percent', 0):.2f}%")
    ...
    if backtest.get("status") == "ok":
        ...
        b1.metric(f"Backtest MAE ({backtest.get('test_days', 0)}d)", f"{backtest.get('mae', 0):.2f}")
        b2.metric("Backtest RMSE", f"{backtest.get('rmse', 0):.2f}")
        b3.metric("Backtest MAPE", f"{backtest.get('mape_percent', 0):.2f}%")
    else:
        st.info("Backtest unavailable for current range. Try selecting a wider date range.")

    st.markdown(f"<h2><span style='color: orange'>{selected_stock}</span> Forecast Plot</h2>", unsafe_allow_html=True)
    st.write(f"This section visualizes the forecasted stock price for {selected_stock} using a time series plot from {end_date} to {end_date + timedelta(days=period)}.")
    ...
    st.markdown(f"<h2><span style='color: orange'>{selected_stock}</span> Forecast Components</h2>", unsafe_allow_html=True)
    st.write(f"This section breaks down the forecast components, including trends and seasonality, for {selected_stock} from {end_date} to {end_date + timedelta(days=period)}.")
...
def render_comparison_tab(selected_stocks: List[str], start_date: date, end_date: date, period: int):
    """Render Comparison tab content."""
    if not selected_stocks:
        st.warning("Please select at least one stock if you want to compare them.")
        return

    from services import load_data, prepare_training_frame, prepare_forecast_table, plot_multiple_data
    ...
    st.markdown(f"<h2><span style='color: orange'>{', '.join(selected_stocks)}</span> Forecast Comparison Plot</h2>", unsafe_allow_html=True)
    st.write(f"This section visualizes the forecasted stock price for {', '.join(selected_stocks)} using a time series plot from {end_date} to {end_date + timedelta(days=period)}.")
    ...
    for i, data in enumerate(stocks_data):
        if data is not None and not data.empty and {"Date", "Close"}.issubset(data.columns):
            ...
            st.markdown(f"<h3><span style='color: orange'>{selected_stocks[i]}</span> Forecast DataFrame</h3>", unsafe_allow_html=True)
...
def render_interview_view(selected_stock: str, config: Dict[str, Any], metrics: Dict[str, float],
                         signal_info: Dict[str, Any], backtest: Dict[str, Any]):
    """Render Interview View tab content."""
    ...
    st.json(
        {
            ...
            "forecast_days": int(config['period']),
            "mae": round(float(metrics.get('mae', 0)), 4),
            "rmse": round(float(metrics.get('rmse', 0)), 4),
            "mape_percent": round(float(metrics.get('mape', 0)), 4),
            "signal": signal_info.get("signal", "UNKNOWN"),
            "expected_return_percent": round(float(signal_info.get('expected_return_percent', 0)), 4),
            "backtest": backtest,
            "status": "healthy",
        }
    )
